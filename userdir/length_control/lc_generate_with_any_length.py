...
def _main(args, output_file):
    ...
    system_out = open(args.system_out, 'w')
    reference = open(args.reference, 'w')
    ...
    for sample in progress:
        ...
        sample['net_input']['tgt_lengths'] = torch.full_like(sample['net_input']['tgt_lengths'], 30)
...
def add_custom_generation_args(parser):
    scriptdir = path.abspath(path.dirname(__file__))
    group = parser.add_argument_group('Generation')
    group.add_argument('--system', dest='system_out',
                       default=path.normpath(path.join(scriptdir, '../../generation/system_output.txt')))
    group.add_argument('--reference', dest='reference',
                       default=path.normpath(path.join(scriptdir, '../../generation/reference.txt')))
    return group


def cli_main():
    env = load_yaml(__file__, '../../script/yaml_configs/env.yml')
    parser = options.get_generation_parser()
    add_custom_generation_args(parser)
